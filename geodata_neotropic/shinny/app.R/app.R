...
if (exists("nc_small", inherits = TRUE)) {
  nc_small <- sf::st_make_valid(nc_small)
}
...
ui <- navbarPage(
  ...
  tabPanel("Map",
           fluidPage(
             ...
             fluidRow(
               column(3,
                      selectInput("color_map", "Color points by",
                                  choices = c("Holdridge_class","Biome","country"),
                                  selected = "Holdridge_class"),
                      checkboxInput("cluster_map", "Cluster markers", value = TRUE),
                      helpText("Capas base Light/Dark + polígonos Morrone")
               ),
               column(9, leafletOutput("map", height = 580))
             )
           )
  ),
...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    ...
    color_var <- "Holdridge_class"   # o "Biome" / "country"
  
    
    # 1) Cargar MORRONE (objeto global 'nc_small' o geojson en data/)
    morrone_sf <- NULL
    if (exists("nc_small", inherits = TRUE)) {
      morrone_sf <- tryCatch(sf::st_transform(nc_small, 4326), error = function(e) NULL)
    } else if (file.exists(file.path("data", "morrone_provinces.geojson"))) {
      morrone_sf <- tryCatch(
        sf::st_read(file.path("data", "morrone_provinces.geojson"), quiet = TRUE) |>
    ...
    # 2) Paletas
    #   - Morrone por IDProv (Set3 expandido al nº de niveles)
    pal_morr <- NULL
    id_levels <- NULL
    if (!is.null(morrone_sf) && "IDProv" %in% names(morrone_sf)) {
      id_levels <- sort(unique(morrone_sf$IDProv))
      pal_morr <- colorFactor(
        palette = colorRampPalette(RColorBrewer::brewer.pal(12, "Set3"))(length(id_levels)),
        domain  = id_levels
      )
    }
    
    #   - Puntos (con tu helper leaf_pal para mantener consistencia)
    col_field  <- df[[color_var]]
    pal_points <- leaf_pal(col_field)
    
    # 3) Base map con estética del ejemplo
    m <- leaflet(options = leafletOptions(minZoom = 2), data = df) %>%
      addMapPane("labels", zIndex = 430) %>%
      addProviderTiles("CartoDB.Positron", group = "Light") %>%
      addProviderTiles("CartoDB.DarkMatter", group = "Dark")
    
    # 4) Polígonos Morrone
    if (!is.null(morrone_sf) && !is.null(pal_morr)) {
      m <- m %>% addPolygons(
        data = morrone_sf,
        fillColor = ~pal_morr(IDProv), fillOpacity = 0.6,
        color = "grey35", weight = 1, smoothFactor = 0.5,
        group = "Morrone Provinces",
        popup = ~paste0(
          "<b>SubDominio:</b> ", ifelse(is.null(SubDominio), "-", as.character(SubDominio)),
          "<br><b>Dominio:</b> ",   ifelse(is.null(Dominio),   "-", as.character(Dominio)),
          "<br><b>IDProv:</b> ",    ifelse(is.null(IDProv),    "-", as.character(IDProv))
        )
      )
    }
    
    # 5) Puntos (grupo “Sampling sites”) — sin cluster por defecto
    m <- m %>% addCircleMarkers(
      lng = ~longitude, lat = ~latitude,
      color = ~pal_points(df[[input$color_map]]),
    ...
    # 6) Leyendas
    m <- m %>% addLegend("bottomright", pal = pal_points, values = col_field,
                         title = input$color_map, opacity = 0.85)
    if (!is.null(morrone_sf) && !is.null(pal_morr)) {
      m <- m %>% addLegend("bottomleft", pal = pal_morr, values = id_levels,
    ...
    m %>% addLayersControl(
      ...
      overlayGroups = c("Sampling sites", if (!is.null(morrone_sf)) "Morrone Provinces"),
      options = layersControlOptions(collapsed = FALSE)
    )
  })
  
  # Alternar clustering sin borrar polígonos
  observeEvent(input$cluster_map, {
    req(input$color_map)
    map_proxy  <- leafletProxy("map", data = df)
    pal_points <- leaf_pal(df[[input$color_map]])
    
    map_proxy %>% clearGroup("Sampling sites")
    
    if (isTRUE(input$cluster_map)) {
      map_proxy %>% addMarkers(
        lng = ~longitude, lat = ~latitude,
