...
its1.f= subset(its1, paper_to_keep=="yes" & morrone_biogeoregions_Region== "Neotropical") 
#Remember: samples were grouped by <90m of proximity
...
#filtering by papers to keep and bioregion
its2.f= subset(its2, paper_to_keep=="yes" & morrone_biogeoregions_Region== "Neotropical") 
#Remember: NEW.id are samples grouped by <90m of proximity and sampling year
...
# 3. Open dataset Andres and filtering ####
# 3.1 Its1
its1.ecm= read.csv("Data/its1_andres.csv", sep=",", fileEncoding="latin1") #same length of its1.f
#filtering by its1.f
to.keep= match(its1.f$PermanentID, its1.ecm$PermanentID) 
its1.ecm.f= its1.ecm[to.keep,] #1688 obs its.f 
unique(its1.ecm.f$morrone_biogeoregions_Region) #check only neotropical

#3.2 its2
its2.ecm= read.csv("Data/its2_andres.csv", sep=",", fileEncoding="latin1") #same length of its1.f
#filtering by its1.f
to.keep2= match(its2.f$PermanentID, its2.ecm$PermanentID) 
its2.ecm.f= its2.ecm[to.keep2,] #1688 obs its.f 
unique(its2.ecm.f$morrone_biogeoregions_Region) #check only neotropical
...
# 4. Figures 
# 4.1 Fig 1.a Histogram of ECM frequency
...
# Same plot with grouped samples 
